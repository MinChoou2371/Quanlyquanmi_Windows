    ...
    public partial class ftable : Form
    {
        ...
        private void BtThanhToan_Click(object sender, EventArgs e) // Button thanh toán
        {
            ...
            else
            {
                if (idBill != -1)
                {
                    if (MessageBox.Show(string.Format("Bạn muốn thực hiện thanh toán cho bàn {0}? \n Tiền nhận: {1} \n Tổng tiền: {2} \n Giảm giá: {3} % \n Thành Tiền: {5} \n Tiền thừa: {4}", table.Name, receive, finalTotalPrice, discount, change,TotalPrice), "Thông báo", MessageBoxButtons.OKCancel) == System.Windows.Forms.DialogResult.OK)
                    {
        ...
        {
            int totalprice = Convert.ToInt32(textBox4.Text.Split(',')[0].Replace(".", ""));
            int temp = totalprice;
            CultureInfo culture = new CultureInfo("vi-VN");
            textBox4.Text = (totalprice ).ToString("c", culture);
        }
    }
}
